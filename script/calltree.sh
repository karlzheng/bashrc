...
build_graphdb()
{
	cscope -bkRu ${GRAPHDB:+-f $GRAPHDB} && echo Created ${GRAPHDB:-cscope.out}...;
}
...
fdefine()
{
	cscope ${GRAPHDB:+-f $GRAPHDB} -d -L1 $1;
}

callees()
{
	cscope ${GRAPHDB:+-f $GRAPHDB} -d -L2 $1;
}

callers()
{
	cscope ${GRAPHDB:+-f $GRAPHDB} -d -L3 $1;
}
...
all_callers()
{
	local tfile=/tmp/all_callers.$RANDOM
	cat ${1:+<(echo $1)} > $tfile
...
all_callees()
{
	local tfile=/tmp/all_callees.$RANDOM
	cat ${1:+<(echo $1)} > $tfile
...
call_tree()
{
	local tfile=/tmp/graph_filter.$RANDOM
...
all_callerees()
{
	callers $1 | filter_cscope_lines | all_callees;
}
...
call_graph()
{
	for a; do for b; do if [ $a != $b ]; then call_tree $a $b; fi; done; done;
	}
...
colornodes()
{
	(cat; for x in $@; do echo "$x [color=red]"; done;)
}
...
_upstream()
{
	all_callers $1 $2 | edges | graph_filter ${3:-caller} | colornodes $1 | graph;
}

_downstream()
{
	all_callees $1 $2 | edges | graph_filter ${3:-callee} | colornodes $1 | graph;
}

_subgraph()
{
	call_tree $1 $2 | edges | graph_filter ${3:-callee} | colornodes $1 $2 | graph;
}

_relate()
{
	call_graph $@ | edges | graph_filter callee | colornodes $@ | graph;
}

_leaks()
{
	call_leaks $1 $2 | edges | graph_filter ${3:-callee} | colornodes $1 $2 | graph;
}
...
upstream()
{
	_upstream $@ > /tmp/tfile; ctviewer /tmp/tfile;
}

downstream()
{
	_downstream $@ > /tmp/tfile; ctviewer /tmp/tfile;
}

subgraph()
{
	_subgraph $@ > /tmp/tfile; ctviewer /tmp/tfile;
}

relate()
{
	_relate $@ > /tmp/tfile; ctviewer /tmp/tfile;
}

leaks()
{
	_leaks $@ > /tmp/tfile; ctviewer /tmp/tfile;
}
...
dot2html()
{
	dot -Tpng -o $1.png -Tcmapx -o $1.map; (echo "<IMG SRC="$1.png" USEMAP="#iftree" />"; cat $1.map)  > $1.html;
}
